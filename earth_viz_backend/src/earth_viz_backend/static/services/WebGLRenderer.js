...
export class WebGLRenderer {
     ...
     */
    setup(type, data, id, globe, useInterpolatedLookup = false) {
    ...
    compileShader(type, projectionType, useInterpolatedLookup = false) {
        if (!this.gl)
            return null;
    ...
    createWeatherTexture(overlayProduct) {
        ...
        if (!texture)
            return null;
        this.gl.bindTexture(this.gl.TEXTURE_2D, texture);
        console.log(`[WebGL] Float textures not supported, using RGBA encoding. Sample values:`, gridData.slice(0, 5), '...', gridData.slice(-5));
    ...
    setProjectionUniforms(program, globe, view) {
        if (!this.gl || !globe.projection)
            return;
    ...
    setOverlayUniforms(program, scaleBounds, textureSize) {
        if (!this.gl)
            return;
