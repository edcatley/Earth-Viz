 ...
 */
export function compositeShader(config) {
    // Choose projection shader based on type
    let projectionShader;
...
export class WebGLSystem {
     ...
     */
    buildProgram(vertexSource, fragmentSource, name) {
    ...
    createTexture(data, config) {
        ...
        if (data instanceof ImageData) {
            this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, data.width, data.height, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, data.data);
        }
        else {
    ...
    updateTexture(texture, data) {
        ...
        try {
            if (data instanceof ImageData) {
                this.gl.texSubImage2D(this.gl.TEXTURE_2D, 0, 0, 0, data.width, data.height, this.gl.RGBA, this.gl.UNSIGNED_BYTE, data.data);
            }
            else {
     ...
     */
    renderLayer(layer, vertexBuffer, canvasSize) {
        if (!this.gl)
            return false;
     ...
     */
    setTextureFromImage(name, image, config) {
        if (!this.gl)
            return false;
     ...
     */
    getProjectionUniforms(globe, view) {
        const uniforms = {};
     ...
     */
    setupOverlay(overlayProduct, overlayId) {
     ...
     */
    renderOverlay(overlayId, globe, view) {
        if (!this.gl || !this.isInitialized) {
            return false;
        }
     ...
     */
    createWeatherDataTexture(overlayProduct) {
        if (!this.gl || !overlayProduct?.forEachPoint)
            return null;
        ...
        overlayProduct.forEachPoint((lon, lat, value) => {
